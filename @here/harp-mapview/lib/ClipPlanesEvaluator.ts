...
export class TopViewClipPlanesEvaluator extends ElevationBasedClipPlanesEvaluator {
    ...
    protected evaluateDistancePlanarProj(camera: THREE.Camera, projection: Projection): ViewRanges {
...
export class TiltViewClipPlanesEvaluator extends TopViewClipPlanesEvaluator {
    /**
     * Cached distance to camera focus (target) point in world units.
     */
    private m_targetDistance: number = 0;
    /**
    ...
    protected getCameraLookAtDistance(camera: THREE.Camera, projection: Projection): number {
        ...
        // cos(tiltAngle) = altitude / targetDistance
        // targetDistance = altitude / cos(tiltAngle)
        // where:
        // cos(tiltAngle) = dot(lookAt, normalInverse)
    ...
    protected evaluateDistancePlanarProj(camera: THREE.Camera, projection: Projection): ViewRanges {
        ...
        this.m_targetDistance = lookAtDist;
    ...
    protected evaluateDistanceSphericalProj(
        ...
        // TODO: Target distance does not need to be cached when we provide full support for
        // target point within all camera tilt angles.
        const target = MapViewUtils.getWorldTargetFromCamera(camera, projection);
        if (target !== null) {
            this.m_targetDistance = camera.position.distanceTo(target);
        }
        // Fallback to camera altitude if target not found and not yet set.
        // NOTE: May be inaccurate when launching map with camera looking to void.
        else if (this.m_targetDistance < cameraAltitude) {
            this.m_targetDistance = cameraAltitude;
        }
        // Apply the constraints.
        const farMin = cameraAltitude - this.minElevation;
        const farMax = this.m_targetDistance * this.farMaxRatio;
        viewRanges.near = Math.max(viewRanges.near, this.nearMin);
